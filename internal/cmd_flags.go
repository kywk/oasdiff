...
func addCommonDiffFlags(cmd *cobra.Command) {
	...
	cmd.PersistentFlags().StringSlice("header", nil, "add a HTTP header to requests when loading specs from URLs (format: key=value, repeatable)")
	cmd.PersistentFlags().StringSlice("query", nil, "add a query parameter to requests when loading specs from URLs (format: key=value, repeatable)")
